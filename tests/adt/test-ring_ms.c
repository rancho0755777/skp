...
static int dequeue_cb(void *_r)
{
	...
	uint64_t cost = cycles_to_ns(escape)/TEST_NR;
...
int main(void)
{
	...
	struct ringb *r = ringb_create(127, RINGB_F_EXACT_SZ|RINGB_F_SC_DEQ
	...
	uint32_t *slot = calloc(TEST_NR+1, sizeof(*slot));
	BUG_ON(!slot);
	r->user = slot+1;

	uthread_t tid[THREAD_NR+1];

...
#define NR_WORK_TRIGGER 128

static uint32_t order_indication = 0U;
...
static struct my_work work_array[128];
...
int main(int argc, char const *argv[])
{
	...
	mywq = alloc_workqueue("mywork", WQ_UNBOUND, 1);
	if (WARN_ON(!mywq))
		return EXIT_FAILURE;
    ...
    WARN_ON(order_indication != NR_WORK_TRIGGER * ARRAY_SIZE(work_array));
